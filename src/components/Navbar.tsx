  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-cream/90 shadow-lg shadow-black/10 backdrop-blur-md"
    ...
    >
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
        <button
          ...
          title="Mohamed & Nashwa"
          aria-label="Mohamed & Nashwa"
        ...
        >
          ...
          <span className="text-gold-dark">M</span>
          ...
          {sections.map((s) => (
            <li key={s.id}>
              <button
                ...
                className="group relative font-body text-sm font-medium text-ink/80 transition hover:text-gold-dark"
              >
                {t(s.key)}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-md border border-gold/40 text-ink md:hidden"
            aria-label="Menu"
          >
        ...
        <ul className="flex flex-col gap-1 border-t border-gold/20 bg-ink/95 px-4 py-3 backdrop-blur-md md:hidden">
          {sections.map((s) => (
            <li key={s.id}>
              <button
                onClick={() => scrollTo(s.id)}
                className="w-full rounded-md px-3 py-2 text-start font-body text-sm font-medium text-cream/90 transition hover:bg-felt/60 hover:text-gold-dark"
              >
                {t(s.key)}
              </button>
            </li>
